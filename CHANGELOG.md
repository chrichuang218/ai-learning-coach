...
- 已掌握知识在新场景中再次出现时只作必要提醒，教学重点转向新的边界、组合与迁移要求，避免把旧知识重新包装成新课。
- 以本地源码为教学锚点时，具体代码讲解和提问提供可点击的绝对路径与当前行号；代码变化后重新定位，避免沿用失效行号。

### Added

- README 增加 Star History，便于查看项目 Star 增长趋势。

## 2.0.2 - 2026-07-26

### Changed

- 将“一次一步”明确为一个有意义的学习动作，避免把完整任务切成连续的字段填空、二选一和原句复述。
- 用户连续两次“不知道”或只能复述提示时，停止同形态追问，改用整体表示、Run 或 Debug 讲清卡点，再回到原任务。
- 提示下局部答对只证明支架有效；减少提示后重新完成完整任务，才可作为独立掌握证据。

### Added

- 增加“连续不知道”和“提示下局部答对”两组粒度回归场景，并收紧 learning-coach rubric 的渐进披露与证据标准。
