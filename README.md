...
遇到真实卡点时，先使用项目自己的测试、Run、Debug、日志或可逆修改。只有这些方式仍无法隔离机制时，才创建一次性临时实验；解决后立即回到真实项目，稳定结论进入 `reference/`，个人证据进入 `learning-records/`。每次写入 record 后都会检查是否应把稳定知识提升或合并到 reference，但不会按文件数量机械对应。不会建立长期练习目录体系。
